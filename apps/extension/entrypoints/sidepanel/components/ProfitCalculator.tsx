...
const num = (v: NumOrBlank): number | undefined =>
  v === "" ? undefined : Number(v);

const pct = (v: NumOrBlank): number | undefined =>
  v === "" ? undefined : Number(v) / 100;

function buildFactor(
  on: boolean,
  value: NumOrBlank,
  convert: (v: NumOrBlank) => number | undefined,
): number | undefined {
  if (!on) return 0;
...
export function ProfitCalculator() {
  ...
  const [manufacturingCost, setManufacturingCost] =
    useState<NumOrBlank>("");
  ...
  const outcome = useMemo(
    () =>
      calcProfit({
        sellingPrice:
          sellingPrice === "" ? NaN : Number(sellingPrice),
        manufacturingCost:
          manufacturingCost === "" ? NaN : Number(manufacturingCost),
  ...
  const visibleKeys = new Set<string>([
    "manufacturing",
    ...activeFactors.flatMap((f) => FACTOR_LINE_KEYS[f]),
  ]);

  if (commissionOn || collectionOn || fixedFeeOn || shippingOn) {
    visibleKeys.add("feeGst");
  }

  const visibleLines =
    result?.lines.filter((line) => visibleKeys.has(line.key)) ?? [];
  ...
  const factorRows: {
  ...
  }[] = [
    {
      ...
      definition: "Marketplace commission.",
    },
    {
      key: "fixedFee",
      label: "Fixed fee (₹)",
    ...
    },
    {
      ...
      definition: "Payment collection fee.",
    },
    {
      key: "shipping",
      label: "Shipping (₹)",
      ...
      definition: "Forward shipping cost.",
    },
    {
      key: "packaging",
      label: "Packaging (₹)",
      ...
      definition: "Packing cost per order.",
    },
    {
      key: "return",
      label: "Return %",
      ...
      definition: "Expected returned-order rate.",
    },
    {
      key: "gst",
      label: "GST %",
      ...
      definition: "GST rate on the product.",
    },
  ];
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-3">
        <h3 className="font-accent text-sm text-black">
          Optional factors
        </h3>
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <label className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`font-cartoon text-[11px] ${
                      row.on
              ...
              {row.on && (
                <p className="mt-1 font-cartoon text-[9px] leading-3.5 text-black/50">
                  {row.definition}
                </p>
              )}
            </div>
      ...
      {result == null ? (
        <p className="mt-3 font-cartoon text-xs text-black/60">
          Enter both values to calculate profit.
        </p>
      ) : (
        <div className="mt-3 rounded-xl border-2 border-black bg-white p-3 shadow-[3px_3px_0px_0px_#000]">
          {result.netProfit > 0 ? (
            ...
            <div className="rounded-lg border-2 border-black bg-[#ff6b6b] px-3 py-2 font-cartoon text-xs font-semibold">
              ⚠ Losing {money(Math.abs(result.netProfit))} — raise
              price above {money(result.breakeven)}
            </div>
          )}
          ...
          <p className="mt-2 font-cartoon text-[9px] leading-4 italic text-black/45">
            TCS/TDS withheld amounts aren't counted as costs.
          </p>
        </div>
      )}
    </div>
