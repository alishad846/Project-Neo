...
// plus Neo's own cost-price tracking.
export const FIELD_DEFS: FieldDef[] = [
  ...
  },
  {
    key: "category",
    label: "Category",
    placeholder: "e.g. Women > Kurtis",
    required: true,
  },
  {
    key: "sku",
    label: "SKU",
    placeholder: "e.g. KURTI-001",
    required: true,
  },
  {
    key: "fabric",
  ...
  },
  {
    key: "brand",
    label: "Brand",
    placeholder: "e.g. NeoDemo",
  },
  {
    key: "styleCode",
    label: "Style / Product ID",
    placeholder: "e.g. STY-1042",
    attribute: true,
  },
  {
    key: "description",
    label: "Description",
    placeholder: "e.g. Breathable pure-cotton kurti for daily wear",
    attribute: true,
  },
  {
    key: "countryOfOrigin",
  ...
  },
  {
    key: "sareeLength",
    label: "Saree Length",
    placeholder: "e.g. 6.3 metres",
    attribute: true,
  },
  {
    key: "notes",
    label: "Notes",
    placeholder: "e.g. Packaging or handling notes",
    attribute: true,
  },
];

const REQUIRED_KEYS = FIELD_DEFS.filter((f) => f.required).map(
  (f) => f.key,
);

// Maps Meesho's field identifiers to this form's FIELD_DEFS keys.
...
export function AddProduct() {
  ...
  async function importFromMeesho() {
    ...
    try {
      ...
      setFields((prev) => {
        ...
        for (const [meeshoKey, formKey] of Object.entries(
          SCRAPE_FIELD_MAP,
        )) {
          if (scraped[meeshoKey]) {
            next[formKey] = scraped[meeshoKey];
          }
        }
      ...
      setMessage("Imported from Meesho. Review the fields before saving.");
    } catch (e) {
  ...
  function missingRequired(): string[] {
    return REQUIRED_KEYS.filter((k) => !fields[k]?.trim()).concat(
  ...
  return (
    <div className="p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <span className="font-body text-[10px] text-black/50">
          Catalogue
        </span>
      </div>
      ...
      <div className="mt-2 grid gap-2 rounded-xl border-2 border-black bg-white p-3 shadow-[3px_3px_0px_0px_#000]">
        ...
        {uploading && (
          <p className="font-cartoon text-[10px] text-black/60">
            Uploading…
          </p>
        )}

        {imageUrls.length > 0 && (
          <p className="font-cartoon text-[10px] text-green-700">
            {imageUrls.length} image(s) uploaded.
          </p>
        )}

        {error && (
          <p className="rounded-lg border-2 border-red-600 bg-red-50 px-2 py-1.5 font-cartoon text-[10px] leading-snug text-red-600">
            {error}
          </p>
        )}

        {message && (
          <p className="rounded-lg border-2 border-green-700 bg-green-50 px-2 py-1.5 font-cartoon text-[10px] leading-snug text-green-700">
            {message}
          </p>
        )}
