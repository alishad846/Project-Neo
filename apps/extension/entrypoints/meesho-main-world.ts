...
export default defineUnlistedScript(() => {
  ...
  window.addEventListener(
    ...
    async (event) => {
      ...
      if (event.data?.type === "PROJECT_NEO_SCRAPE_MEESHO") {
        const { requestId } = event.data;

        try {
          const autofillApi = (window as any).meeshoAutofill;

          if (!autofillApi || typeof autofillApi.scrapeProduct !== "function") {
            throw new Error("Meesho scrape API is unavailable.");
          }

          const fields = autofillApi.scrapeProduct();

          window.postMessage(
            {
              source: "PROJECT_NEO_MEESHO_MAIN",
              type: "PROJECT_NEO_SCRAPE_RESULT",
              requestId,
