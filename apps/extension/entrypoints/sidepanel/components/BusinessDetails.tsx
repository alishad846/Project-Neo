    ...
    <div className="p-4">
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        {BUSINESS_FIELD_GROUPS.map((group) => (
          ...
          >
            <legend className="px-1 font-accent text-sm text-[#ff90e8]">
              {group.title}
            </legend>
            ...
            <div className="grid gap-2">
              {group.fields.map((f) => (
                <label
                  key={f.key}
                  className="font-cartoon text-[11px] font-semibold leading-4"
                >
                  {f.label}
      ...
      <button
        type="button"
        onClick={save}
        className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-black bg-[#b2ff59] px-3 py-2.5 font-cartoon text-sm font-bold shadow-[3px_3px_0px_0px_#000] transition-all hover:-translate-y-0.5 active:translate-y-0 active:shadow-none"
      >
        {saved ? (
