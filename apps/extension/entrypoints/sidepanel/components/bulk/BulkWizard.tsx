...
export function BulkWizard() {
  ...
  if (!sku) {
    return (
      <p className="p-4 font-cartoon text-xs text-black/60">
        No SKUs to show.
      </p>
    );
  }
  ...
  async function generate() {
    ...
    try {
      ...
      if (measurementCols.length) {
        for (const [si, s] of skus.entries()) {
          for (const [vi, v] of s.variants.entries()) {
            if (
              optionalGroupStatus(v.measurements, measurementCols) ===
              "partial"
            ) {
              setStatus(
                `SKU ${si + 1}, variant ${vi + 1}: fill all measurement fields or leave them blank.`,
              );
              setIndex(si);
              return;
            }
      ...
      if (!res.success) {
        setStatus(
          res.error ||
            (res.validationProblems ?? [])
              .map((p) => `Row ${p.row} — ${p.field}: ${p.error}`)
              .join("\n") ||
            "Generation failed.",
        );
        return;
      ...
      if (res.blobBytes) {
        const blob = new Blob([res.blobBytes], {
          type:
            res.blobType ||
        ...
        a.href = url;
        a.download = res.filename || "Project-Neo-Meesho-Bulk.xlsx";
      ...
      // Save each SKU to Manage Catalogue. Download remains independent.
      ...
      setStatus(
        `Done — ${res.rows ?? products.length} row(s) generated and saved.`,
      );
    } catch (e) {
  ...
  return (
    <div className="p-4">
      <div className="mb-2 flex items-center justify-between">
        <span className="font-cartoon text-[11px] font-semibold text-black/60">
          SKU {index + 1} of {skus.length}
        </span>
      ...
      <div className="mt-3 flex gap-2">
        <button
          ...
          className="rounded-lg border-2 border-black bg-white px-3 py-2 font-cartoon text-xs font-semibold disabled:opacity-40"
        >
          ← Back
        </button>
        ...
        {!isLast ? (
          <button
            type="button"
            onClick={() => setIndex((i) => i + 1)}
            className="rounded-lg border-2 border-black bg-[#8bd3ff] px-3 py-2 font-cartoon text-xs font-semibold"
          >
            Next →
          </button>
        ) : (
      ...
      {status && (
        <p className="mt-2 whitespace-pre-wrap rounded-lg bg-white px-2.5 py-2 font-cartoon text-[11px] leading-4 text-black/70">
          {status}
        </p>
      )}
    </div>
...
function Step1InputsBridge(props: {
  ...
  return (
    <Step1Inputs
      onReady={(schema, skus) => {
        if (metaRef.current) {
          props.onReady(schema, skus, metaRef.current);
        }
      }}
